...
<div class="main-content">
	<?php
	if (isset($_SESSION['id']) && $_SESSION['identity'] !== 'Admin') {
		include 'db.php';
		$id = $_SESSION['id'];
		$sql = "SELECT saftyquestion FROM user WHERE id = '".$id."'";
		$result = mysqli_query($conn, $sql);
		$row = mysqli_fetch_assoc($result);

		if ($row['saftyquestion'] == NULL) {
			echo '<div style="color: red; text-align: center; margin-top: 20px;">';
			echo '<strong>Notice:</strong> You have not set a safety question. Please update your Profile.';
			echo '</div>';
		}
	}
	?>
...
if ($_SESSION['identity'] === 'Student') {
    // Important - Only late submissions (submitted but overdue)
    $sql = "SELECT s.id, s.assessment_id, s.submitted_at, s.mark, f.title 
            FROM submissions s
            JOIN files f ON s.assessment_id = f.id
            WHERE s.student_id = ? 
            AND s.submitted_at > f.deadline
    ...
    // Recent activity - Only submitted in past month
    $sql = "SELECT s.id, s.assessment_id, s.submitted_at, s.mark, f.title 
            FROM submissions s
            JOIN files f ON s.assessment_id = f.id
            WHERE s.student_id = ? 
    ...
    // Upcoming - Assessments to hand in (not submitted yet)
    $sql = "SELECT f.id, f.title, f.deadline 
            FROM files f
            WHERE f.file_type = 'assessment' 
            AND f.deadline > NOW()
            AND f.id NOT IN (
        ...
        <table style="width: 100%; border-collapse: collapse;">
            ...
            <?php if (!empty($importantSubmissions)): ?>
                <?php foreach ($importantSubmissions as $sub): ?>
                    <tr>
                        <td><strong><?= htmlspecialchars($sub['title']) ?></strong></td>
                        <td><span style="color: #e74c3c;">Late Submission</span></td>
                        <td><?= date('M j, Y', strtotime($sub['submitted_at'])) ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="3" style="padding: 10px; text-align: center;">No late submissions</td></tr>
            <?php endif; ?>
            <tr><td colspan="3"><br></td></tr>  <!-- Added line break -->
            
            <!-- Recent Activity - Submissions from past month -->
            <tr>
            ...
            <?php if (!empty($recentSubmissions)): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($sub['mark'] !== null): ?>
                                <span style="color: #27ae60;">Graded: <?= htmlspecialchars($sub['mark']) ?></span>
                            <?php else: ?>
                                <span style="color: #f39c12;">Submitted</span>
                            <?php endif; ?>
                        </td>
                        <td><?= date('M j, Y', strtotime($sub['submitted_at'])) ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="3" style="padding: 10px; text-align: center;">No recent submissions</td></tr>
            <?php endif; ?>
            <tr><td colspan="3"><br></td></tr>  <!-- Added line break -->
            
            <!-- Upcoming - Assessments to hand in -->
            <tr>
                <th colspan="3"><header><h3><p align="left">Upcoming Deadlines</p></h3></header></th>
            </tr>
            <tr><td colspan="3"><hr></td></tr>
            <?php if (!empty($upcomingReminders)): ?>
                <?php foreach ($upcomingReminders as $reminder): ?>
                    <tr>
                        <td><strong><?= htmlspecialchars($reminder['title']) ?></strong></td>
                        <td>Due: <?= date('M j, Y', strtotime($reminder['deadline'])) ?></span></td>
                        <td>
                                <?= (strtotime($reminder['deadline']) - time()) < (3 * 24 * 60 * 60) ? 'Urgent!' : 'Pending' ?>
                            </span>
                        </td>
                    </tr>
                <?php endforeach; ?>
